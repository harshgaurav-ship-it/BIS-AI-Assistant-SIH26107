...
with st.sidebar:
    ...
    st.write("**Current milestone:** Evidence-first assistant")
    st.write("**Knowledge base:** Local FAISS index ready")
    st.write("**Answer safety:** Evidence threshold enabled")
...
question = st.chat_input(
    "Ask about BIS standards, certification, or hallmarking..."
)
...
if question:
    ...
    if product_hint:
        full_question = (
            f"{question} "
            f"(related product/standard: {product_hint})"
        )
    ...
    with st.chat_message("assistant"):
        with st.spinner("Checking verified BIS sources..."):
            try:
                ...
                st.error(
                    "Something went wrong while checking BIS sources: "
                    f"{error}"
                )
                st.stop()
